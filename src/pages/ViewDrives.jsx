  ...
  drives.forEach((drive, index) => {
    ...
    autoTable(doc, {
      ...
      body: drive.driveDetails.map((d) => [
        d.message || "",
        d.reminder?.date ? new Date(d.reminder.date).toLocaleDateString() : "",
    ...
    drive.driveClosingDetails.forEach((closing) => {
      doc.text(`Closing Message: ${closing.closingMessage || ""}`, 10, yOffset);
      yOffset += 6;
        ...
        {drives.length > 0 ? (
          drives.map((drive) => (
            <Link
              ...
              key={drive._id}
              className={`border-2 p-4 rounded-lg transition duration-200 ${getBorderColor(drive.status)}`}
            >
              <h3 className="text-2xl font-semibold">{drive.companyName}</h3>
              <p><strong>Coordinator:</strong> {drive.coodName}</p>
              <p><strong>Status:</strong> {drive.status}</p>
              <p><strong>Phone:</strong> {drive.phoneNumber}</p>
              <p><strong>Date Created:</strong> {drive.dateCreated}</p>
              <p><strong>Date Updated:</strong> {drive.dateUpdated}</p>
            </Link>
          ))
        ) : (
